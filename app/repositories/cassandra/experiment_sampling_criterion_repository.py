...
class ExperimentSamplingCriterionModel(Model):
    ...
    experiment_id = columns.UUID(primary_key=True)
    sampling_model = columns.Text(required=True)
    sampling_attribute = columns.Text(required=True)
    created_at = columns.DateTime(default=datetime.now)
    updated_at = columns.DateTime(default=datetime.now)


class ExperimentSamplingCriterionRepository:
    ...
    def find_by_id(self, experiment_sampling_criterion_id: UUID) -> Optional[ExperimentSamplingCriterion]:
        criterion = ExperimentSamplingCriterionModel.objects(id=experiment_sampling_criterion_id).allow_filtering().first()
        if not criterion:
            return None
